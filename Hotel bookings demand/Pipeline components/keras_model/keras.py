...
def kerasmodel(clean_data):
    ...
    weights = keras_model.save_weights('kerasmodel.h5')
    report = metrics.classification_report(y_test, y_pred, output_dict=True)
    df_classification_report = pd.DataFrame(report).transpose()
    print(df_classification_report)

    keras_metrics = {'loss':loss, 'test':accuracy,                     'report':df_classification_report, 
                    'model': [model_json, weights]}
